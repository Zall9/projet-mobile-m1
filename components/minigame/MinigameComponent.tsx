...
export default function MinigameComponent() {
  ...
  const [loop, setLoop] = useState<AsyncIterable<number>>();
  useEffect(() => {
      ...
      {minigame && logicGridToUpdate.size ? (
        <Box id="Canvas">
          ...
          <IconButton
            aria-label="left"
            icon={<ArrowLeftIcon />}
            onClick={() => minigame.playerInput("left", logicGridToUpdate)}
          />
          <IconButton
            aria-label="right"
            icon={<ArrowRightIcon />}
            onClick={() => minigame.playerInput("right", logicGridToUpdate)}
          />
        </Box>
      ) : (
        <></>
