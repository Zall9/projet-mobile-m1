(() => {
var exports = {};
exports.id = 964;
exports.ids = [964];
exports.modules = {

/***/ 3744:
/***/ ((module, __unused_webpack_exports, __webpack_require__) => {

var map = {
	"./Memory.tsx": 4941,
	"./Panier.tsx": 6369,
	"./RPS.tsx": 8870,
	"./Unknown.tsx": 7179
};


function webpackContext(req) {
	var id = webpackContextResolve(req);
	return __webpack_require__(id);
}
function webpackContextResolve(req) {
	if(!__webpack_require__.o(map, req)) {
		var e = new Error("Cannot find module '" + req + "'");
		e.code = 'MODULE_NOT_FOUND';
		throw e;
	}
	return map[req];
}
webpackContext.keys = function webpackContextKeys() {
	return Object.keys(map);
};
webpackContext.resolve = webpackContextResolve;
module.exports = webpackContext;
webpackContext.id = 3744;

/***/ }),

/***/ 477:
/***/ ((module, __webpack_exports__, __webpack_require__) => {

"use strict";
__webpack_require__.a(module, async (__webpack_handle_async_dependencies__, __webpack_async_result__) => { try {
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "Z": () => (/* binding */ MinigameComponent)
/* harmony export */ });
/* harmony import */ var react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(997);
/* harmony import */ var react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _chakra_ui_react__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(8930);
/* harmony import */ var _chakra_ui_react__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(6689);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _model_Minigames_MinigameController__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(791);
/* harmony import */ var _model_Minigames_MinigameList_Unknown__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(7179);
/* harmony import */ var next_router__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(1853);
/* harmony import */ var next_router__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(next_router__WEBPACK_IMPORTED_MODULE_5__);
/* harmony import */ var _model_Leaderboard__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(1538);
var __webpack_async_dependencies__ = __webpack_handle_async_dependencies__([_model_Leaderboard__WEBPACK_IMPORTED_MODULE_6__]);
_model_Leaderboard__WEBPACK_IMPORTED_MODULE_6__ = (__webpack_async_dependencies__.then ? (await __webpack_async_dependencies__)() : __webpack_async_dependencies__)[0];







function MinigameComponent() {
    let router = (0,next_router__WEBPACK_IMPORTED_MODULE_5__.useRouter)();
    let logicGrid = new Map();
    let miniGameId;
    let { 0: minigame , 1: setMinigame  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(_model_Minigames_MinigameList_Unknown__WEBPACK_IMPORTED_MODULE_4__.minigameInfos);
    let { 0: reset , 1: sr  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(false);
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(()=>{
        miniGameId = localStorage.getItem("miniGameId");
        setMinigame(miniGameId === "random" ? _model_Minigames_MinigameController__WEBPACK_IMPORTED_MODULE_3__/* .MinigameController.pickRandomMinigame */ .l.pickRandomMinigame() : _model_Minigames_MinigameController__WEBPACK_IMPORTED_MODULE_3__/* .MinigameController.getById */ .l.getById(miniGameId));
        if (minigame === _model_Minigames_MinigameList_Unknown__WEBPACK_IMPORTED_MODULE_4__.minigameInfos) {
            sr(!reset);
            return;
        }
        let rowTemplate = [];
        for(let i = 0; i < minigame.nbRow; i++){
            rowTemplate.push(minigame.caseTemplateCreate());
        }
        for(let i1 = 0; i1 < minigame.nbCol; i1++){
            logicGrid.set(String.fromCharCode(65 + i1), JSON.parse(JSON.stringify(rowTemplate)));
        }
        minigame.score = 0;
        minigame.init(logicGrid, setUpdateGrid, setLogicGrid);
        setLogicGrid(logicGrid);
        setUpdateGrid(true);
    }, [
        reset
    ]);
    let { 0: updateGrid , 1: setUpdateGrid  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(false);
    let { 0: logicGridToUpdate , 1: setLogicGrid  } = (0,react__WEBPACK_IMPORTED_MODULE_2__.useState)(logicGrid);
    //@ts-ignore
    (0,react__WEBPACK_IMPORTED_MODULE_2__.useEffect)(()=>{
        //@ts-ignore
        const myInterval = setInterval(()=>{
            minigame.evolve(logicGridToUpdate);
        }, minigame.refreshInterval);
        if (updateGrid) {
            if (minigame.score >= minigame.scoreTresh) {
                const ldb = new _model_Leaderboard__WEBPACK_IMPORTED_MODULE_6__/* .Leaderboard */ .s();
                ldb.init().then(()=>{
                    const user = ldb.getUser(localStorage.getItem("username"));
                    user.score += minigame.scoreTresh;
                    _model_Leaderboard__WEBPACK_IMPORTED_MODULE_6__/* .Leaderboard.updateUser */ .s.updateUser(user);
                    router.push("/game").then(()=>null);
                });
            }
            minigame.update(logicGridToUpdate);
            setUpdateGrid(false);
        }
        //@ts-ignore
        return ()=>clearInterval(myInterval);
    }, [
        updateGrid
    ]);
    return /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.Fragment, {
        children: minigame && logicGridToUpdate.size ? /*#__PURE__*/ (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.Fragment, {
            children: [
                /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_1__.Center, {
                    children: /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_1__.Box, {
                        id: "Grid",
                        children: minigame.ViewGrid(logicGridToUpdate)
                    })
                }),
                minigame.Controls(logicGridToUpdate)
            ]
        }) : /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.Fragment, {})
    });
}

__webpack_async_result__();
} catch(e) { __webpack_async_result__(e); } });

/***/ }),

/***/ 821:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "Nm": () => (/* binding */ Direction),
/* harmony export */   "Of": () => (/* binding */ Unknown)
/* harmony export */ });
/* unused harmony export IOutput */
class IOutput {
}
var Direction;
(function(Direction) {
    Direction[Direction["up"] = 0] = "up";
    Direction[Direction["down"] = 1] = "down";
    Direction[Direction["left"] = 2] = "left";
    Direction[Direction["right"] = 3] = "right";
})(Direction || (Direction = {}));
const Unknown = {
    nextEvent: "",
    willMinigame: false,
    direction: Direction.up
};


/***/ }),

/***/ 791:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "Z": () => (/* binding */ getStaticPropsMinigame),
/* harmony export */   "l": () => (/* binding */ MinigameController)
/* harmony export */ });
let minigameIdList;
async function getStaticPropsMinigame() {
    const { readdirSync  } = await Promise.resolve(/* import() */).then(__webpack_require__.t.bind(__webpack_require__, 7147, 23));
    return readdirSync("./model/Minigames/MinigameList/").map((file)=>{
        return file.slice(0, -4);
    });
}
class MinigameController {
    static init(initializer) {
        minigameIdList = initializer.slice();
    }
    static getById(id) {
        return (minigameIdList.includes(id) ? __webpack_require__(3744)(`./${id}.tsx`) : __webpack_require__(7179)).minigameInfos;
    }
    static pickRandomMinigame() {
        const validMinigames = minigameIdList.filter((v)=>v !== "Unknown");
        return this.getById(validMinigames[Math.floor(Math.random() * validMinigames.length)]);
    }
}


/***/ }),

/***/ 4941:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "MemoryCaseTemplate": () => (/* binding */ MemoryCaseTemplate),
  "minigameInfos": () => (/* binding */ minigameInfos)
});

// EXTERNAL MODULE: external "react/jsx-runtime"
var jsx_runtime_ = __webpack_require__(997);
// EXTERNAL MODULE: external "@chakra-ui/react"
var react_ = __webpack_require__(8930);
// EXTERNAL MODULE: external "@chakra-ui/icons"
var icons_ = __webpack_require__(4513);
;// CONCATENATED MODULE: ./components/icons/MemoryIcons.tsx


const BackIcon = (0,icons_.createIcon)({
    displayName: "BackIcon",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ (0,jsx_runtime_.jsxs)("g", {
            children: [
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    fill: "#8A3FFC",
                    d: "M376,0H136c-22.056,0-40,17.944-40,40v432c0,22.056,17.944,40,40,40h240c22.056,0,40-17.944,40-40V40 C416,17.944,398.056,0,376,0z M400,472c0,13.234-10.766,24-24,24H136c-13.234,0-24-10.766-24-24V40c0-13.233,10.766-24,24-24h240 c13.234,0,24,10.767,24,24V472z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    fill: "#8A3FFC",
                    d: "M376,87.998V48c0-4.418-3.582-8-8-8h-39.997c-0.002,0-0.003,0-0.005,0h-39.995c-0.002,0-0.003,0-0.005,0h-39.995 c-0.002,0-0.003,0-0.005,0H144c-4.418,0-8,3.582-8,8v15.998c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005v39.995 c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005v39.995 c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005v39.995c0,0.001,0,0.003,0,0.005V464 c0,4.418,3.582,8,8,8h224c4.418,0,8-3.582,8-8v-15.998c0-0.001,0-0.003,0-0.005v-39.995c0-0.001,0-0.003,0-0.005v-39.995 c0-0.001,0-0.003,0-0.005v-39.995c0-0.001,0-0.003,0-0.005v-39.995c0-0.001,0-0.003,0-0.005v-39.995c0-0.001,0-0.003,0-0.005 v-39.995c0-0.001,0-0.003,0-0.005v-39.995c0-0.001,0-0.003,0-0.005v-39.995c0-0.001,0-0.003,0-0.005V88.002 C376,88.001,376,87.999,376,87.998z M307.314,456L152,300.687v-17.373L324.686,456H307.314z M267.313,456L152,340.687v-17.373 L284.686,456H267.313z M227.313,456L152,380.687v-17.373L244.687,456H227.313z M187.313,456L152,420.687v-17.373L204.687,456 H187.313z M152,203.313l208,208v17.373l-208-208V203.313z M152,180.687v-17.373l208,208v17.373L152,180.687z M152,140.687v-17.373 l208,208v17.373L152,140.687z M152,100.687V83.313l208,208v17.373L152,100.687z M204.686,56L360,211.313v17.373L187.314,56H204.686 z M227.314,56h17.373L360,171.313v17.373L227.314,56z M267.313,56h17.373L360,131.313v17.373L267.313,56z M307.313,56h17.373 L360,91.313v17.373L307.313,56z M360,68.687L347.313,56H360V68.687z M164.686,56L360,251.313v17.373l-208-208V56H164.686z M152,443.313L164.687,456H152V443.313z M347.314,456L152,260.687v-17.373l208,208V456H347.314z"
                })
            ]
        })
    })
});
const Memory1 = (0,icons_.createIcon)({
    displayName: "Memory1",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.884 3.717l47.393 58.27-47.393 58.286-47.375-58.287 47.375-58.27zm100.541 136.816c9.59 0 18.465 2.061 26.627 6.182 8.242 4.12 14.424 9.549 18.545 16.285 4.121 6.657 6.18 13.948 6.18 21.873 0 5.468-1.03 11.016-3.09 16.643-2.06 5.626-5.07 10.897-9.033 15.81-6.815 8.48-13.83 15.73-21.041 21.752l-31.383 26.39c-7.925 6.658-13.71 12.323-17.355 16.999-3.646 4.675 14.344 7.013 53.968 7.013 5.944 0 10.183-1.544 12.719-4.634 2.536-3.09 5.429-11.651 8.678-25.678h8.084l-3.805 49.332h-106.51v-6.418l34.592-39.348c19.257-20.921 30.828-34.076 34.711-39.464 5.547-7.688 9.59-15.099 12.125-22.23 2.536-7.212 3.805-13.75 3.805-19.614 0-9.748-2.695-17.394-8.084-22.942-5.39-5.626-12.878-8.441-22.467-8.441-10.778 0-19.573 2.575-26.389 7.727-6.815 5.15-10.224 10.619-10.224 16.404 0 1.981.516 3.487 1.547 4.517.95 1.03 3.01 1.942 6.18 2.735 8.003 1.902 12.007 6.578 12.007 14.027 0 4.042-1.349 7.41-4.043 10.104-2.694 2.615-6.142 3.923-10.342 3.923-4.834 0-9.073-2.22-12.718-6.658-3.646-4.517-5.47-10.38-5.47-17.592 0-8.083 2.062-15.414 6.182-21.992 4.2-6.657 10.5-12.085 18.9-16.285 8.401-4.28 17.436-6.42 27.104-6.42zm118.801 175.99l47.393 58.27-47.393 58.287-47.375-58.287 47.375-58.27z"
        })
    })
});
const Memory2 = (0,icons_.createIcon)({
    displayName: "Memory2",
    viewBox: "0 0 800 800",
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.654 8.045s46.336 33.84 47.27 63.07c.775 24.287-25.025 32.12-40.776 18.584l13.633 32.653h-40.115l13.613-32.633c-15.535 13.88-40.006 5.347-40.758-18.606-.88-28.01 47.133-63.066 47.133-63.066v-.002zm98.736 120.959c14.978 0 28.647 3.804 41.01 11.412 12.363 7.529 22.27 18.385 29.719 32.57 7.449 14.106 11.174 29.916 11.174 47.43 0 16.008-2.933 30.274-8.797 42.795-5.785 12.521-14.898 23.972-27.34 34.353 1.268 3.25 2.654 7.688 4.16 13.315 2.774 10.54 5.429 17.394 7.965 20.564 2.536 3.17 5.428 4.754 8.678 4.754 3.011 0 5.586-1.703 7.726-5.111 2.219-3.329 3.408-8.914 3.567-16.76h7.132c-.079 14.265-2.854 24.764-8.322 31.5-5.468 6.736-12.6 10.106-21.396 10.106-8.163 0-14.82-3.29-19.971-9.868-5.072-6.498-8.44-19.018-10.104-37.562-4.438 1.11-8.717 1.941-12.838 2.496-4.12.555-8.044.832-11.77.832-14.74 0-28.37-3.804-40.892-11.412-12.442-7.687-22.346-18.663-29.716-32.928-7.291-14.344-10.936-30.51-10.936-48.5 0-25.518 8.28-47.31 24.844-65.379 15.057-16.404 33.76-24.607 56.107-24.607zm.356 9.035c-9.827 0-18.663 2.773-26.508 8.32-7.766 5.548-13.869 14.226-18.307 26.034-4.358 11.728-6.537 27.698-6.537 47.906 0 14.264 1.307 26.507 3.922 36.73 2.615 10.223 6.697 19.93 12.244 29.123-.158-3.645-.238-6.576-.238-8.795 0-9.826 3.132-17.91 9.392-24.25 6.34-6.34 14.621-9.51 24.844-9.51 10.302 0 18.94 3.448 25.914 10.342 5.23 5.23 10.144 14.304 14.74 27.221 4.28-8.083 7.41-16.839 9.391-26.27 1.981-9.43 2.973-21.24 2.973-35.425 0-19.97-2.18-35.74-6.54-47.31-4.358-11.65-10.5-20.248-18.425-25.796-7.925-5.547-16.88-8.32-26.865-8.32zm-1.545 114.83c-7.45 0-13.552 2.378-18.307 7.133-4.755 4.755-7.13 10.895-7.13 18.424 0 7.45 2.932 14.503 8.796 21.16 5.627 2.457 11.53 3.685 17.711 3.685 4.755 0 9.232-.594 13.432-1.783a46.814 46.814 0 0 0 11.888-5.23c-.158-14.265-2.021-24.606-5.588-31.026-4.517-8.241-11.451-12.363-20.802-12.363zm102.142 64.78h40.116L372.262 386.3c15.75-13.536 41.551-5.701 40.775 18.586-.933 29.23-47.271 63.068-47.271 63.068s-48.01-35.059-47.131-63.068c.751-23.953 25.222-32.484 40.758-18.604l-13.614-32.635z"
        })
    })
});
const Memory3 = (0,icons_.createIcon)({
    displayName: "Memory3",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.434 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5zm57.216 16.174c13.613-.319 26.504 9.854 27.075 28.043.976 31.09-47.74 52.945-52.313 70.015-4.997-18.649-51.413-37.573-52.45-70.015-.994-31.155 37.404-37.907 52.452-11.846 6.262-10.846 15.923-15.978 25.236-16.195zm70.21 124.361c9.588 0 18.464 2.06 26.626 6.182 8.242 4.12 14.424 9.549 18.545 16.285 4.121 6.657 6.182 13.948 6.182 21.873 0 5.468-1.031 11.014-3.092 16.64-2.06 5.627-5.07 10.898-9.033 15.811-6.815 8.48-13.83 15.731-21.041 21.754l-31.383 26.39c-7.925 6.657-13.708 12.323-17.353 16.999-3.646 4.675 14.342 7.013 53.966 7.013 5.944 0 10.183-1.546 12.72-4.636 2.535-3.09 5.428-11.65 8.677-25.676h8.084l-3.805 49.332H189.445v-6.42l34.592-39.346c19.257-20.921 30.826-34.078 34.71-39.466 5.547-7.688 9.588-15.097 12.124-22.229 2.536-7.211 3.805-13.749 3.805-19.613 0-9.748-2.695-17.396-8.084-22.944-5.39-5.626-12.878-8.44-22.467-8.44-10.778 0-19.573 2.576-26.389 7.727-6.815 5.152-10.222 10.62-10.222 16.405 0 1.981.514 3.487 1.545 4.517.95 1.03 3.011 1.942 6.181 2.735 8.004 1.902 12.006 6.576 12.006 14.025 0 4.042-1.347 7.411-4.04 10.106-2.695 2.615-6.143 3.921-10.343 3.921-4.834 0-9.075-2.218-12.72-6.656-3.646-4.517-5.467-10.382-5.467-17.594 0-8.083 2.059-15.412 6.18-21.99 4.2-6.657 10.501-12.085 18.902-16.285 8.4-4.28 17.433-6.42 27.101-6.42zM360.728 361.77c4.572 17.07 53.287 38.925 52.31 70.015-.57 18.189-13.462 28.362-27.074 28.043v-.002c-9.314-.217-18.975-5.349-25.236-16.195-15.048 26.061-53.446 19.31-52.452-11.846 1.037-32.442 47.454-51.366 52.452-70.016z"
        })
    })
});
const Memory4 = (0,icons_.createIcon)({
    displayName: "Memory4",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.654 8.045s46.338 33.84 47.271 63.07c.776 24.287-25.026 32.12-40.777 18.584l13.633 32.653h-40.115l13.613-32.633c-15.535 13.88-40.006 5.347-40.758-18.606-.88-28.01 47.133-63.066 47.133-63.066v-.002zm99.687 132.49c9.59 0 18.465 2.06 26.627 6.182 8.242 4.12 14.424 9.549 18.545 16.285 4.121 6.657 6.182 13.946 6.182 21.871 0 5.468-1.031 11.016-3.092 16.643-2.06 5.626-5.07 10.897-9.033 15.81-6.816 8.48-13.83 15.731-21.041 21.754l-31.383 26.389c-7.925 6.657-13.71 12.322-17.355 16.998v.002c-3.646 4.675 14.344 7.011 53.968 7.011 5.944 0 10.183-1.544 12.72-4.634 2.535-3.091 5.428-11.651 8.677-25.678h8.084L294.87 344.5h-106.51v-6.418l34.592-39.348c19.257-20.921 30.828-34.076 34.711-39.464 5.547-7.688 9.59-15.097 12.125-22.229 2.536-7.212 3.805-13.75 3.805-19.615 0-9.748-2.695-17.394-8.084-22.942-5.39-5.626-12.878-8.44-22.467-8.44-10.778 0-19.573 2.576-26.389 7.727-6.815 5.152-10.224 10.62-10.224 16.405 0 1.981.516 3.487 1.547 4.517.95 1.03 3.01 1.94 6.18 2.733 8.003 1.902 12.007 6.578 12.007 14.027 0 4.042-1.348 7.41-4.043 10.104-2.694 2.615-6.142 3.923-10.342 3.923-4.834 0-9.073-2.218-12.718-6.656-3.646-4.517-5.47-10.382-5.47-17.594 0-8.083 2.061-15.414 6.182-21.992 4.2-6.657 10.5-12.085 18.9-16.285 8.401-4.28 17.436-6.418 27.104-6.418zM345.78 353.648h40.116l-13.633 32.653c15.75-13.536 41.551-5.701 40.775 18.586-.933 29.23-47.271 63.068-47.271 63.068s-48.01-35.059-47.131-63.068c.751-23.953 25.222-32.485 40.758-18.604l-13.614-32.635z"
        })
    })
});
const Memory5 = (0,icons_.createIcon)({
    displayName: "Memory5",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.884 3.717h.002l47.393 58.271-47.393 58.287-47.377-58.289 47.375-58.27zm100.303 136.818c14.899 0 26.786 4.002 35.662 12.006 8.876 8.004 13.315 17.91 13.315 29.719 0 8.48-2.418 15.809-7.252 21.99-4.755 6.181-12.996 11.65-24.725 16.404 11.332 2.774 20.168 7.37 26.508 13.79 8.083 8.162 12.125 18.266 12.125 30.312 0 13.63-4.834 24.567-14.502 32.808-11.015 9.51-25.359 14.266-43.031 14.266-10.62 0-19.774-1.743-27.461-5.23-7.608-3.487-13.314-8.203-17.117-14.147-3.804-5.943-5.706-12.638-5.706-20.088 0-6.023 1.467-10.778 4.399-14.265s6.617-5.23 11.055-5.23c4.041 0 7.29 1.346 9.748 4.04 2.456 2.615 3.683 6.222 3.683 10.819 0 2.932-.632 5.427-1.9 7.488-1.268 1.981-3.489 3.844-6.658 5.588-3.408 1.822-5.112 3.566-5.112 5.23 0 2.378 1.19 5.07 3.567 8.082 2.457 2.932 6.023 5.31 10.699 7.133 4.755 1.743 10.738 2.615 17.95 2.615 12.441 0 21.634-3.05 27.577-9.152 6.023-6.181 9.035-16.048 9.035-29.6 0-14.185-2.577-24.25-7.728-30.193-5.072-6.023-12.639-9.035-22.703-9.035-2.695 0-6.064.317-10.106.95-3.249.476-5.585.714-7.011.714-2.378 0-4.241-.475-5.588-1.426-1.268-1.03-1.903-2.299-1.903-3.805 0-1.664.595-2.93 1.784-3.802 1.268-.951 3.288-1.428 6.062-1.428l6.54.238 5.704.238c10.065 0 17.633-2.575 22.705-7.726 5.072-5.151 7.608-13.117 7.608-23.895 0-13.234-2.418-22.624-7.252-28.172-4.834-5.626-12.046-8.439-21.635-8.439-8.797 0-15.809 1.584-21.04 4.754-5.23 3.17-7.845 6.102-7.845 8.797 0 1.743 1.506 4.081 4.518 7.013 4.28 3.963 6.418 8.084 6.418 12.364 0 3.249-1.19 5.982-3.567 8.2-2.377 2.22-5.427 3.33-9.152 3.33-3.962 0-7.212-1.505-9.748-4.517-2.457-3.011-3.686-7.212-3.686-12.601 0-8.4 3.013-15.85 9.036-22.348 8.558-9.193 20.801-13.789 36.73-13.789zm119.04 175.988h.001l47.39 58.272-47.39 58.287-47.377-58.289 47.375-58.27z"
        })
    })
});
const Memory6 = (0,icons_.createIcon)({
    displayName: "Memory6",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm57.214 16.174h.002c13.612-.318 26.504 9.855 27.075 28.043.976 31.09-47.74 52.945-52.313 70.015-4.997-18.649-51.413-37.573-52.45-70.015-.994-31.155 37.404-37.91 52.452-11.848 6.261-10.846 15.921-15.977 25.234-16.195zm69.973 124.361c14.899 0 26.786 4.002 35.662 12.006 8.876 8.004 13.315 17.909 13.315 29.717 0 8.48-2.418 15.81-7.252 21.992-4.755 6.181-12.996 11.65-24.725 16.404 11.333 2.774 20.168 7.37 26.508 13.79 8.083 8.162 12.125 18.266 12.125 30.312 0 13.63-4.834 24.567-14.502 32.808-11.016 9.51-25.36 14.266-43.033 14.266-10.62 0-19.772-1.743-27.46-5.23-7.607-3.487-13.313-8.203-17.116-14.147-3.804-5.943-5.706-12.64-5.706-20.09 0-6.023 1.467-10.776 4.399-14.263s6.617-5.23 11.055-5.23c4.041 0 7.29 1.346 9.748 4.04 2.456 2.615 3.683 6.222 3.683 10.819 0 2.932-.634 5.427-1.902 7.488-1.268 1.981-3.487 3.844-6.656 5.588-3.408 1.822-5.112 3.566-5.112 5.23 0 2.378 1.19 5.07 3.567 8.082 2.457 2.932 6.023 5.31 10.699 7.133 4.755 1.743 10.738 2.615 17.95 2.615 12.441 0 21.634-3.052 27.577-9.154 6.023-6.181 9.033-16.046 9.033-29.598 0-14.185-2.575-24.25-7.726-30.193-5.072-6.023-12.639-9.035-22.703-9.035-2.695 0-6.064.317-10.106.95-3.249.476-5.587.714-7.013.714-2.378 0-4.24-.475-5.586-1.426-1.268-1.03-1.903-2.299-1.903-3.805 0-1.664.595-2.93 1.784-3.802 1.268-.951 3.288-1.428 6.062-1.428l6.537.238 5.707.238c10.065 0 17.633-2.575 22.705-7.726 5.072-5.151 7.608-13.117 7.608-23.895 0-13.234-2.418-22.624-7.252-28.172-4.834-5.626-12.046-8.439-21.635-8.439-8.797 0-15.809 1.584-21.04 4.754-5.23 3.17-7.845 6.102-7.845 8.797 0 1.743 1.504 4.081 4.516 7.013 4.28 3.963 6.42 8.082 6.42 12.362 0 3.249-1.19 5.984-3.567 8.203-2.377 2.219-5.427 3.328-9.152 3.328-3.962 0-7.212-1.506-9.748-4.518-2.457-3.01-3.686-7.21-3.686-12.6 0-8.4 3.013-15.848 9.036-22.347 8.558-9.193 20.801-13.789 36.73-13.789zM360.73 361.77c4.572 17.07 53.289 38.923 52.312 70.013-.57 18.189-13.462 28.364-27.074 28.045v-.002c-9.314-.217-18.975-5.349-25.237-16.195-15.048 26.061-53.445 19.307-52.45-11.848 1.036-32.442 47.451-51.364 52.449-70.013z"
        })
    })
});
const Memory7 = (0,icons_.createIcon)({
    displayName: "Memory7",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.652 8.047s46.338 33.838 47.271 63.068c.776 24.287-25.024 32.12-40.775 18.584l13.633 32.655h-40.115l13.613-32.635c-15.535 13.88-40.006 5.347-40.758-18.606-.88-28.01 47.13-63.066 47.13-63.066zm99.451 132.488c14.899 0 26.786 4.002 35.662 12.006 8.876 8.004 13.313 17.91 13.313 29.719 0 8.48-2.416 15.809-7.25 21.99-4.755 6.181-12.998 11.65-24.727 16.404 11.333 2.774 20.17 7.37 26.51 13.79 8.083 8.162 12.125 18.266 12.125 30.312 0 13.63-4.836 24.567-14.504 32.808-11.015 9.51-25.359 14.266-43.031 14.266-10.62 0-19.772-1.743-27.46-5.23-7.607-3.487-13.313-8.203-17.116-14.147-3.804-5.943-5.707-12.64-5.707-20.09 0-6.023 1.466-10.776 4.398-14.263 2.932-3.487 6.617-5.23 11.055-5.23 4.041 0 7.291 1.346 9.748 4.04 2.456 2.615 3.685 6.222 3.685 10.819 0 2.932-.634 5.427-1.902 7.488-1.268 1.981-3.486 3.844-6.656 5.588-3.408 1.822-5.112 3.566-5.112 5.23 0 2.378 1.19 5.07 3.567 8.082 2.456 2.932 6.021 5.31 10.697 7.133 4.755 1.743 10.738 2.615 17.95 2.615 12.441 0 21.636-3.052 27.58-9.154 6.022-6.181 9.032-16.046 9.032-29.598 0-14.185-2.575-24.25-7.726-30.193-5.072-6.023-12.64-9.035-22.705-9.035-2.695 0-6.062.317-10.104.95-3.249.476-5.587.714-7.013.714-2.378 0-4.24-.475-5.586-1.426-1.268-1.03-1.903-2.299-1.903-3.805 0-1.664.595-2.93 1.784-3.802 1.268-.951 3.288-1.428 6.062-1.428l6.537.238 5.707.238c10.065 0 17.631-2.575 22.703-7.726 5.072-5.151 7.61-13.117 7.61-23.895 0-13.234-2.418-22.624-7.252-28.172-4.834-5.626-12.046-8.439-21.635-8.439-8.797 0-15.81 1.584-21.041 4.754-5.23 3.17-7.846 6.102-7.846 8.797 0 1.743 1.506 4.081 4.518 7.013 4.28 3.963 6.42 8.084 6.42 12.364 0 3.249-1.19 5.982-3.567 8.2-2.377 2.22-5.43 3.33-9.154 3.33-3.962 0-7.21-1.507-9.746-4.519-2.457-3.01-3.686-7.21-3.686-12.6 0-8.4 3.01-15.848 9.034-22.347 8.558-9.193 20.803-13.789 36.732-13.789zm100.238 177.113h40.118l-13.633 32.653c15.75-13.536 41.551-5.701 40.775 18.586-.933 29.23-47.271 63.068-47.271 63.068s-48.01-35.059-47.131-63.068c.751-23.953 25.222-32.485 40.758-18.604l-13.616-32.635z"
        })
    })
});
const Memory8 = (0,icons_.createIcon)({
    displayName: "Memory8",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.832 8.408v.002h.015c13.587.01 24.442 10.853 24.442 24.442 0 5.71-2.003 10.997-5.266 15.173 1.12-.158 2.232-.31 3.396-.31 13.595 0 24.458 11.157 24.458 24.752 0 13.594-10.863 24.752-24.458 24.752-5.76 0-11.08-2.048-15.294-5.422l12.609 30.193h-40.117l12.42-29.744c-4.12 3.115-9.233 4.973-14.776 4.973-13.594 0-24.752-11.16-24.752-24.754 0-13.595 11.158-24.752 24.752-24.752 1.059 0 2.058.184 3.082.312-3.215-4.16-5.248-9.509-5.248-15.173 0-13.589 11.15-24.434 24.737-24.444zM269.67 179.625h7.726v112.453h27.342v10.104h-27.342v25.32c0 2.219.872 4 2.616 5.348 2.536 1.822 5.468 2.734 8.797 2.734h10.103v8.916H232.94v-8.916h12.243c4.12 0 7.133-.872 9.035-2.615 1.981-1.744 2.972-4.28 2.972-7.608v-23.18h-67.283V290.77l79.764-111.145zm-12.48 32.213l-57.417 80.24h57.416v-80.24zm88.605 142.174h40.117l-12.611 30.193c4.215-3.374 9.535-5.422 15.295-5.422 13.594 0 24.459 11.158 24.459 24.752 0 13.595-10.865 24.752-24.46 24.752-1.163 0-2.275-.152-3.396-.31 3.263 4.176 5.266 9.462 5.266 15.173 0 13.589-10.853 24.433-24.44 24.442h-.017c-13.588-.01-24.735-10.853-24.735-24.442 0-5.665 2.033-11.013 5.248-15.173-1.023.128-2.025.312-3.084.312-13.594 0-24.751-11.159-24.751-24.754 0-13.594 11.157-24.752 24.752-24.752 5.542 0 10.655 1.858 14.775 4.973l-12.418-29.744z"
        })
    })
});
const Memory9 = (0,icons_.createIcon)({
    displayName: "Memory9",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.882 3.717h.002l47.393 58.27-47.393 58.286-47.375-58.287 47.373-58.27zm124.436 139.908h7.726v112.453h27.34v10.104h-27.34v25.32c0 2.219.872 4.002 2.616 5.35 2.536 1.822 5.468 2.734 8.797 2.734h10.103v8.914h-65.975v-8.914h12.245c4.12 0 7.133-.872 9.035-2.615 1.981-1.744 2.97-4.28 2.97-7.608v-23.181h-67.28v-11.41l79.763-111.147zM258.27 211.84l-57.414 80.238h57.414V211.84zm107.39 140.683h.001l47.393 58.27-47.393 58.287-47.375-58.287 47.373-58.27z"
        })
    })
});
const Memory10 = (0,icons_.createIcon)({
    displayName: "Memory10",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm57.214 16.174h.002c13.612-.318 26.502 9.855 27.073 28.043.976 31.09-47.738 52.945-52.31 70.015-4.998-18.649-51.416-37.573-52.452-70.015-.995-31.155 37.403-37.91 52.451-11.848 6.261-10.846 15.923-15.977 25.236-16.195zm94.104 127.451h7.726v112.453h27.34v10.104h-27.34v25.32c0 2.219.872 4.002 2.616 5.35 2.536 1.822 5.468 2.734 8.797 2.734h10.103v8.914h-65.975v-8.914h12.245c4.12 0 7.133-.872 9.035-2.615 1.981-1.744 2.97-4.28 2.97-7.608v-23.181h-67.28v-11.41l79.763-111.147zM258.27 211.84l-57.414 80.238h57.414V211.84zM360.73 361.77c4.572 17.07 53.287 38.923 52.31 70.013-.57 18.188-13.46 28.36-27.072 28.043h-.002c-9.313-.218-18.975-5.35-25.236-16.195-15.048 26.061-53.444 19.307-52.45-11.848 1.037-32.442 47.452-51.364 52.45-70.013z"
        })
    })
});
const Memory11 = (0,icons_.createIcon)({
    displayName: "Memory11",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5v-363c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.832 8.408v.002h.015c13.587.01 24.442 10.853 24.442 24.442 0 5.71-2.003 10.997-5.266 15.173 1.12-.158 2.232-.31 3.396-.31 13.595 0 24.458 11.157 24.458 24.752 0 13.594-10.863 24.752-24.458 24.752-5.76 0-11.08-2.048-15.294-5.422l12.609 30.193h-40.117l12.42-29.744c-4.12 3.115-9.233 4.973-14.776 4.973-13.594 0-24.752-11.16-24.752-24.754 0-13.595 11.158-24.752 24.752-24.752 1.059 0 2.058.184 3.082.312-3.215-4.16-5.248-9.509-5.248-15.173 0-13.589 11.15-24.434 24.737-24.444zm63.61 132.127c20.05 4.597 36.415 6.895 49.095 6.895 5.943 0 11.531-.557 16.761-1.666 5.23-1.19 9.153-2.536 11.768-4.041 1.347-.714 2.219-1.07 2.615-1.07.793 0 1.19.594 1.19 1.783 0 2.773-1.784 6.46-5.35 11.056-3.566 4.517-8.44 8.122-14.621 10.817-6.102 2.694-13.156 4.04-21.16 4.04-9.272 0-20.328-1.426-33.166-4.279l-3.922 45.41c6.023-3.01 11.768-5.27 17.236-6.775 5.468-1.506 10.66-2.26 15.572-2.26 15.533 0 28.57 5.151 39.11 15.453 10.62 10.303 15.928 23.022 15.928 38.159 0 16.8-5.626 30.629-16.88 41.486-11.253 10.857-25.835 16.285-43.745 16.285-13.71 0-24.884-3.882-33.522-11.648-8.559-7.846-12.838-17.158-12.838-27.936 0-6.498 1.347-11.49 4.041-14.976 2.774-3.567 6.3-5.35 10.58-5.35 4.2 0 7.608 1.426 10.223 4.28 2.694 2.773 4.043 6.418 4.043 10.935 0 3.407-.674 6.063-2.022 7.965-1.267 1.902-3.645 3.486-7.132 4.754-3.487 1.347-5.23 2.893-5.23 4.636 0 2.536 1.188 5.35 3.566 8.44 2.377 3.011 5.944 5.509 10.699 7.49 4.834 1.902 10.342 2.852 16.523 2.852 12.125 0 21.358-3.765 27.697-11.293 6.42-7.529 9.63-19.772 9.63-36.73 0-15.454-3.37-26.747-10.106-33.88-6.736-7.132-15.334-10.699-25.795-10.699-4.755 0-9.509.872-14.264 2.615-4.755 1.744-9.51 4.359-14.265 7.846-.555 5.547-1.545 9.272-2.971 11.174-1.426 1.822-3.25 2.734-5.469 2.734-1.585 0-2.853-.554-3.804-1.664-.951-1.11-1.426-2.654-1.426-4.635 0-3.487 1.624-6.936 4.873-10.343.95-1.03 1.625-1.94 2.021-2.733l2.735-34.95c1.11-13.788 1.704-27.18 1.783-40.177zm135.917 177.477h40.117l-12.611 30.193c4.215-3.374 9.535-5.422 15.295-5.422 13.594 0 24.459 11.157 24.459 24.752 0 13.595-10.865 24.752-24.46 24.752-1.163 0-2.275-.152-3.396-.31 3.263 4.176 5.266 9.462 5.266 15.173 0 13.589-10.853 24.433-24.44 24.442h-.017c-13.588-.01-24.735-10.853-24.735-24.442 0-5.665 2.033-11.013 5.248-15.173-1.023.128-2.025.312-3.084.312-13.594 0-24.751-11.159-24.751-24.754 0-13.594 11.157-24.752 24.752-24.752 5.542 0 10.655 1.858 14.775 4.973l-12.418-29.744z"
        })
    })
});
const Memory12 = (0,icons_.createIcon)({
    displayName: "Memory12",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#fbfbfbfb",
            d: "M119.436 36c-16.126 0-29.2 17.237-29.2 38.5v363.002c0 21.263 13.074 38.5 29.2 38.5h275.298c16.126 0 29.198-17.237 29.198-38.5V74.5c0-21.263-13.072-38.5-29.198-38.5H119.436zm26.884 3.719l47.393 58.27-47.393 58.286-47.375-58.287 47.375-58.27zm64.64 136.816c20.05 4.597 36.415 6.895 49.095 6.895 5.943 0 11.531-.555 16.761-1.664 5.23-1.19 9.153-2.536 11.768-4.041 1.347-.714 2.219-1.07 2.615-1.07.793 0 1.19.594 1.19 1.782 0 2.774-1.784 6.459-5.35 11.055-3.566 4.517-8.44 8.124-14.621 10.819-6.102 2.694-13.154 4.04-21.158 4.04-9.272 0-20.328-1.426-33.166-4.279l-3.924 45.41c6.023-3.011 11.768-5.271 17.236-6.777 5.468-1.506 10.66-2.258 15.573-2.258 15.532 0 28.569 5.151 39.109 15.453 10.62 10.303 15.93 23.022 15.93 38.159 0 16.8-5.628 30.629-16.881 41.486-11.253 10.857-25.834 16.285-43.744 16.285-13.71 0-24.884-3.882-33.522-11.648-8.559-7.846-12.838-17.158-12.838-27.936 0-6.498 1.347-11.491 4.041-14.978 2.774-3.567 6.3-5.348 10.58-5.348 4.2 0 7.608 1.426 10.223 4.28 2.694 2.773 4.041 6.418 4.041 10.935 0 3.407-.674 6.063-2.022 7.965-1.268 1.902-3.643 3.486-7.13 4.754-3.487 1.347-5.23 2.893-5.23 4.636 0 2.536 1.188 5.349 3.566 8.44 2.377 3.011 5.942 5.507 10.697 7.488 4.834 1.902 10.342 2.853 16.523 2.853 12.125 0 21.358-3.764 27.698-11.292 6.419-7.53 9.628-19.772 9.628-36.731 0-15.453-3.367-26.747-10.103-33.879-6.736-7.132-15.336-10.7-25.797-10.7-4.755 0-9.509.873-14.264 2.616-4.755 1.744-9.51 4.359-14.265 7.846-.555 5.547-1.545 9.272-2.97 11.174-1.427 1.822-3.25 2.734-5.47 2.734-1.585 0-2.851-.555-3.802-1.664-.951-1.11-1.428-2.656-1.428-4.637 0-3.487 1.626-6.934 4.875-10.342.95-1.03 1.623-1.94 2.02-2.732l2.734-34.95c1.11-13.788 1.704-27.182 1.783-40.179zm154.7 175.988h.002l47.393 58.272-47.393 58.287-47.375-58.287 47.373-58.272z"
        })
    })
});

// EXTERNAL MODULE: external "react"
var external_react_ = __webpack_require__(6689);
var external_react_default = /*#__PURE__*/__webpack_require__.n(external_react_);
;// CONCATENATED MODULE: ./components/event/ennemy.tsx



const Ennemy = (props)=>{
    const name = props.name;
    const [health, setHealth] = external_react_default().useState(3);
    return /*#__PURE__*/ (0,jsx_runtime_.jsxs)(react_.Box, {
        sx: {
            display: "flex",
            flexDirection: "column",
            alignItems: "center"
        },
        children: [
            /*#__PURE__*/ jsx_runtime_.jsx(react_.Text, {
                children: "Health"
            }),
            /*#__PURE__*/ jsx_runtime_.jsx(react_.Text, {
                children: health
            }),
            /*#__PURE__*/ jsx_runtime_.jsx(react_.Image, {
                src: "/ennemies/" + name + ".png",
                w: "min(25vh,25vw)"
            })
        ]
    });
};
/* harmony default export */ const ennemy = (Ennemy);

;// CONCATENATED MODULE: ./model/Minigames/MinigameList/Memory.tsx




class MemoryCaseTemplate {
    idLink = -1;
    constructor(){
        this.isReturned = false;
        this.isFound = false;
    }
}
// Faire une liste de 36 images à faire apparaître dans le jeu de memory
const imageList = [
    /*#__PURE__*/ jsx_runtime_.jsx(jsx_runtime_.Fragment, {}),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory1, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory2, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory3, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory4, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory5, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory6, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory7, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory8, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory9, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory10, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory11, {
        boxSize: 12
    }),
    /*#__PURE__*/ jsx_runtime_.jsx(Memory12, {
        boxSize: 12
    }), 
];
const backImage = /*#__PURE__*/ jsx_runtime_.jsx(BackIcon, {
    boxSize: 12
});
function getReturnedCells(logicGrid) {
    const rc = [];
    logicGrid.forEach((row)=>{
        row.forEach((cell)=>{
            const realCell = cell;
            if (realCell.isReturned) rc.push(realCell);
        });
    });
    return rc;
}
function transformCoord(index, nbCol) {
    const col = String.fromCharCode("A".charCodeAt(0) + index % nbCol);
    const row = Math.floor(index / nbCol).toString();
    return col + row;
}
let visuClick = true;
const minigameInfos = {
    Controls () {
        return /*#__PURE__*/ jsx_runtime_.jsx(jsx_runtime_.Fragment, {});
    },
    refreshInterval: 99999999999999,
    setUpdateGrid: ()=>null,
    setLogicGrid: ()=>null,
    nbRow: 6,
    nbCol: 4,
    ViewGrid (logicGrid) {
        const cols = Array.from(logicGrid.keys());
        const parameters = [];
        for(let i = 0; i < this.nbRow; i++){
            for(let j = 0; j < this.nbCol; j++){
                const observedCell = logicGrid.get(cols[j])[i];
                parameters.push(observedCell.isFound || observedCell.isReturned ? imageList[observedCell.idLink] : backImage);
            }
        }
        return /*#__PURE__*/ (0,jsx_runtime_.jsxs)(react_.Grid, {
            gap: 6,
            h: "min(70vh,70vw)",
            w: "min(80vh,80vw)",
            templateColumns: `repeat(${this.nbCol},auto)`,
            children: [
                parameters.map((v, index)=>/*#__PURE__*/ jsx_runtime_.jsx(react_.Center, {
                        children: /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                            colSpan: 1,
                            rowSpan: 1,
                            w: "auto",
                            h: "auto",
                            onClick: ()=>this.playerInput(transformCoord(index, this.nbCol), logicGrid),
                            children: v
                        })
                    })),
                /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                    colSpan: 1
                }),
                /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                    colSpan: this.nbCol - 2,
                    children: /*#__PURE__*/ jsx_runtime_.jsx(ennemy, {
                        name: "bonefire"
                    })
                }),
                /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                    colSpan: 1
                })
            ]
        });
    },
    score: 0,
    scoreTresh: 6 * 4 / 2,
    player: {
        position: {
            x: 0,
            y: "A"
        },
        direction: []
    },
    init (logicGrid, setUpdateGrid, setLogicGrid) {
        const values = Array.from({
            length: this.nbRow * this.nbCol / 2
        }, (_, i)=>i + 1);
        values.push(...Array.from({
            length: this.nbRow * this.nbCol / 2
        }, (_, i)=>i + 1));
        for(let i = values.length - 1; i > 0; i--){
            const j = Math.floor(Math.random() * (i + 1));
            [values[i], values[j]] = [
                values[j],
                values[i]
            ];
        }
        logicGrid.forEach((row, i)=>{
            row.forEach((cell, j)=>{
                let realCell = cell;
                realCell.idLink = values[i.charCodeAt(0) - "A".charCodeAt(0) + this.nbCol * j];
            });
        });
        this.setUpdateGrid = setUpdateGrid;
        this.setLogicGrid = setLogicGrid;
    },
    update (logicGrid) {
        this.setLogicGrid(logicGrid);
        this.setUpdateGrid(true);
    },
    caseTemplateCreate: ()=>new MemoryCaseTemplate(),
    playerInput (input, logicGrid) {
        // input de la forme "COLROW"
        let returnedCells = getReturnedCells(logicGrid);
        if (!visuClick) {
            for (const returnedCell of returnedCells){
                returnedCell.isReturned = false;
            }
            visuClick = true;
        }
        const returnedCell1 = logicGrid.get(input[0])[parseInt(input[1])];
        console.log(returnedCell1);
        returnedCell1.isReturned = true;
        returnedCells = getReturnedCells(logicGrid);
        if (returnedCells.length > 1) {
            if (returnedCells[0].idLink === returnedCells[1].idLink) {
                returnedCells[0].isFound = true;
                returnedCells[1].isFound = true;
                this.score += 1;
            }
            visuClick = false;
        }
        this.update(logicGrid);
        this.setUpdateGrid(true);
    },
    evolve () {}
};


/***/ }),

/***/ 6369:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "PanierCaseTemplate": () => (/* binding */ PanierCaseTemplate),
  "minigameInfos": () => (/* binding */ minigameInfos)
});

// EXTERNAL MODULE: external "react/jsx-runtime"
var jsx_runtime_ = __webpack_require__(997);
// EXTERNAL MODULE: ./model/Events/IOutput.ts
var IOutput = __webpack_require__(821);
// EXTERNAL MODULE: external "@chakra-ui/react"
var react_ = __webpack_require__(8930);
// EXTERNAL MODULE: external "@chakra-ui/icons"
var icons_ = __webpack_require__(4513);
;// CONCATENATED MODULE: ./components/icons/Panier.tsx


// using `path`
const Panier = (0,icons_.createIcon)({
    displayName: "UpDownIcon",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ (0,jsx_runtime_.jsxs)("svg", {
        version: "1.1",
        viewBox: "-1 -1 61 38",
        xmlns: "http://www.w3.org/2000/svg",
        children: [
            /*#__PURE__*/ (0,jsx_runtime_.jsxs)("g", {
                id: "Bowl",
                transform: "matrix(0.70652174949646, 0, 0, 0.70652174949646, 0, 0)",
                children: [
                    /*#__PURE__*/ (0,jsx_runtime_.jsxs)("g", {
                        id: "OutsideBowl_1_",
                        children: [
                            /*#__PURE__*/ jsx_runtime_.jsx("path", {
                                id: "ID0.22138935374096036",
                                fill: "#65CBEB",
                                d: "M 46.800 69.291 C 46.139 69.291 45.768 69.275 45.750 69.274 C 0.619 64.974 1.458 36.392 1.516 35.177 C 1.709 31.129 3.693 26.995 5.155 24.462 C 7.328 27.459 11.461 29.786 17.464 31.388 C 22.888 32.836 31.086 35.025 45.140 35.149 L 46.056 35.153 C 46.457 35.153 83.430 35.017 89.898 23.999 C 91.997 27.410 92.921 34.276 93.014 38.621 C 86.417 67.809 53.373 69.291 46.800 69.291 C 46.801 69.291 46.800 69.291 46.800 69.291 L 46.800 69.291 Z ",
                                transform: "matrix(0.6302494406700134, 0, 0, 0.6302494406700134, 0.15, -6.95)"
                            }),
                            /*#__PURE__*/ jsx_runtime_.jsx("path", {
                                id: "ID0.7706678751856089",
                                d: "M 89.596 26.812 C 90.778 30.070 91.414 34.950 91.510 38.470 C 85.066 66.373 53.155 67.790 46.800 67.790 C 46.328 67.790 46.013 67.782 45.883 67.777 C 28.783 66.135 16.473 60.881 9.293 52.157 C 2.920 44.415 2.945 36.713 3.015 35.248 C 3.153 32.351 4.301 29.373 5.453 27.067 C 8.004 29.520 11.897 31.454 17.078 32.838 C 22.576 34.306 30.884 36.524 45.128 36.650 C 45.431 36.652 45.740 36.654 46.057 36.654 C 49.977 36.654 80.218 36.338 89.596 26.812 M 5.640 21.105 C 5.640 21.105 5.640 21.106 5.640 21.105 L 5.517 21.105 C 5.517 21.105 0.350 28.105 0.017 35.105 C -0.316 42.105 3.684 66.772 45.684 70.772 C 45.684 70.772 46.080 70.790 46.800 70.790 C 53.444 70.790 87.751 69.298 94.517 38.772 C 94.517 38.772 94.290 24.552 89.321 21.162 C 88.150 30.842 60.240 33.653 46.056 33.653 C 45.748 33.653 45.448 33.652 45.153 33.649 C 31.289 33.526 23.174 31.360 17.851 29.939 C 12.528 28.518 7.851 26.272 5.876 22.817 C 5.534 22.221 5.489 21.647 5.640 21.105 L 5.640 21.105 L 89.596 26.812 Z ",
                                transform: "matrix(0.6302494406700134, 0, 0, 0.6302494406700134, 0.15, -6.95)"
                            })
                        ]
                    }),
                    /*#__PURE__*/ (0,jsx_runtime_.jsxs)("g", {
                        id: "InsideBowl",
                        children: [
                            /*#__PURE__*/ jsx_runtime_.jsx("path", {
                                id: "ID0.219700216781348",
                                fill: "#660000",
                                d: "M 45.007 35.149 C 30.980 35.018 22.797 32.698 17.382 31.163 C 11.519 29.501 7.585 27.071 6.004 24.135 C 5.856 23.860 5.822 23.629 5.893 23.362 C 6.235 22.055 8.760 20.504 10.962 19.614 L 11.212 19.516 C 21.819 15.761 33.761 13.845 46.642 13.845 C 65.926 13.845 81.503 18.152 84.701 19.636 C 87.860 21.103 88.631 22.660 88.796 23.120 C 87.384 31.418 62.989 35.154 45.922 35.154 L 45.007 35.149 L 45.007 35.149 Z ",
                                transform: "matrix(0.6302494406700134, 0, 0, 0.6302494406700134, 0.15, -6.95)"
                            }),
                            /*#__PURE__*/ jsx_runtime_.jsx("path", {
                                id: "ID0.2999734110198915",
                                d: "M 46.642 15.344 C 65.846 15.344 81.384 19.750 84.069 20.996 C 86.078 21.929 86.896 22.834 87.208 23.298 C 86.257 26.166 80.848 28.964 72.462 30.878 C 63.500 32.924 52.876 33.653 45.921 33.653 C 45.614 33.653 45.314 33.652 45.021 33.649 C 31.195 33.520 23.129 31.233 17.792 29.720 C 12.458 28.208 8.881 26.096 7.424 23.600 C 7.818 23.017 9.296 21.912 11.479 21.023 L 11.536 21.002 L 11.585 20.981 L 11.611 20.970 L 11.782 20.907 C 22.258 17.216 33.985 15.344 46.642 15.344 M 46.642 12.344 C 35.385 12.344 22.847 13.804 10.643 18.127 C 10.643 18.127 10.622 18.135 10.593 18.147 C 10.588 18.149 10.583 18.150 10.577 18.153 C 10.538 18.167 10.485 18.189 10.414 18.217 C 10.410 18.219 10.404 18.221 10.399 18.223 C 9.337 18.652 5.088 20.507 4.441 22.980 C 4.286 23.571 4.332 24.196 4.682 24.846 C 6.709 28.611 11.509 31.057 16.972 32.606 C 22.435 34.155 30.763 36.516 44.992 36.649 C 45.294 36.652 45.604 36.653 45.920 36.653 C 60.477 36.653 89.118 33.590 90.320 23.042 C 90.320 23.042 90.060 20.470 85.332 18.274 C 82.315 16.875 66.467 12.344 46.642 12.344 L 46.642 12.344 L 46.642 15.344 Z ",
                                transform: "matrix(0.6302494406700134, 0, 0, 0.6302494406700134, 0.15, -6.95)"
                            })
                        ]
                    })
                ]
            }),
            /*#__PURE__*/ (0,jsx_runtime_.jsxs)("g", {
                id: "Bowl_Front",
                children: [
                    /*#__PURE__*/ jsx_runtime_.jsx("path", {
                        id: "ID0.31010090559720993",
                        fill: "#CC7700",
                        d: "M 47.886 69.821 C 46.538 69.821 45.759 69.776 45.742 69.775 C 0.102 65.427 0.958 36.388 1.017 35.153 C 1.233 30.614 3.634 25.992 5.131 23.523 C 7.082 26.735 11.273 29.218 17.593 30.905 C 22.993 32.347 31.154 34.526 45.144 34.649 L 46.056 34.653 C 46.464 34.653 85.085 34.512 89.875 22.949 C 93.455 26.740 93.917 34.938 93.523 38.667 C 90.465 67.572 57.822 69.821 47.886 69.821 C 47.887 69.821 47.887 69.821 47.886 69.821 L 47.886 69.821 Z ",
                        transform: "matrix(0.4226316511631012, 0, 0, 0.4226316511631012, 1.1, -2.75)"
                    }),
                    /*#__PURE__*/ jsx_runtime_.jsx("path", {
                        id: "ID0.6804001186974347",
                        d: "M 90.014 24.759 C 92.334 28.345 92.944 34.630 92.528 38.563 C 89.558 66.638 57.608 68.820 47.886 68.820 C 46.723 68.820 45.989 68.786 45.833 68.778 C 28.436 67.115 15.882 61.737 8.520 52.793 C 1.914 44.768 1.942 36.731 2.015 35.201 C 2.188 31.576 3.841 27.867 5.222 25.360 C 7.561 28.161 11.624 30.347 17.335 31.872 C 22.784 33.327 31.018 35.525 45.135 35.650 C 45.435 35.652 45.741 35.654 46.055 35.654 C 47.679 35.654 82.667 35.519 90.014 24.759 M 5.640 21.105 C 5.640 21.105 5.640 21.106 5.640 21.105 L 5.517 21.105 C 5.517 21.105 0.350 28.105 0.017 35.105 C -0.316 42.105 3.684 66.772 45.684 70.772 C 45.684 70.772 46.487 70.819 47.886 70.819 C 56.988 70.819 91.339 68.817 94.517 38.772 C 94.999 34.207 94.290 24.552 89.321 21.162 C 88.150 30.842 60.240 33.653 46.056 33.653 C 45.748 33.653 45.448 33.652 45.153 33.649 C 31.289 33.526 23.174 31.360 17.851 29.939 C 12.528 28.518 7.851 26.272 5.876 22.817 C 5.534 22.221 5.489 21.647 5.640 21.105 L 5.640 21.105 L 90.014 24.759 Z ",
                        transform: "matrix(0.4226316511631012, 0, 0, 0.4226316511631012, 1.1, -2.75)"
                    })
                ]
            })
        ]
    })
});

;// CONCATENATED MODULE: ./components/icons/Cerise.tsx


// using `path`
const Cerise = (0,icons_.createIcon)({
    displayName: "UpDownIcon",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        version: "1.1",
        id: "Layer_1",
        x: "0px",
        y: "0px",
        children: /*#__PURE__*/ (0,jsx_runtime_.jsxs)("g", {
            children: [
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#9AD14B"
                    },
                    d: "M330.81,7.5c0,30.1-12.2,57.34-31.92,77.06c-19.73,19.72-46.97,31.92-77.07,31.92 C221.82,56.29,270.62,7.5,330.81,7.5z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#F95428"
                    },
                    d: "M206.97,276.56c10.63-16.27,26.2-26.54,43.54-26.54c12.44,0,23.95,5.3,33.4,14.29 c9.44-8.99,20.95-14.29,33.39-14.29c32.04,0,58,35.02,58,78.22c0,43.19-25.96,78.21-58,78.21c-1.86,0-3.71-0.12-5.53-0.35 c-18.53-2.37-37.2-2.37-55.73,0c-1.82,0.23-3.66,0.35-5.53,0.35c-17.34,0-32.91-10.27-43.53-26.54 c-9.01-13.78-14.47-31.86-14.47-51.67C192.51,308.43,197.97,290.34,206.97,276.56z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("g", {
                    children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
                        style: {
                            fill: "#E54728"
                        },
                        d: "M257.977,379.91c-9.01-13.78-14.47-31.86-14.47-51.67s5.46-37.9,14.46-51.68 c5.017-7.679,11.135-14.02,18.02-18.577c-7.692-5.09-16.331-7.963-25.478-7.963c-17.34,0-32.91,10.27-43.54,26.54 c-9,13.78-14.46,31.87-14.46,51.68s5.46,37.89,14.47,51.67c10.62,16.27,26.19,26.54,43.53,26.54c1.87,0,3.71-0.12,5.53-0.35 c10.711-1.37,21.468-1.922,32.219-1.708C276.325,400.629,265.805,391.903,257.977,379.91z"
                    })
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#F95428"
                    },
                    d: "M206.98,379.91c-3.51,5.36-7.54,10.07-12,13.97c-9.08,7.96-19.9,12.57-31.53,12.57 c-1.88,0-3.72-0.11-5.53-0.35c-18.53-2.37-37.21-2.37-55.73,0c-1.82,0.24-3.67,0.35-5.53,0.35c-32.04,0-58.01-35.01-58.01-78.22 c0-43.19,25.97-78.2,58.01-78.2c12.43,0,23.95,5.29,33.39,14.28c9.45-8.99,20.95-14.28,33.4-14.28c11.63,0,22.45,4.61,31.52,12.56 c4.46,3.9,8.5,8.61,12,13.97c-9,13.78-14.46,31.87-14.46,51.68S197.97,366.13,206.98,379.91z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("g", {
                    children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
                        style: {
                            fill: "#E54728"
                        },
                        d: "M104.117,379.91c-9.01-13.78-14.47-31.86-14.47-51.67s5.46-37.9,14.46-51.68 c5.017-7.679,11.135-14.02,18.02-18.577c-7.692-5.09-16.331-7.963-25.478-7.963c-17.34,0-32.91,10.27-43.54,26.54 c-9,13.78-14.46,31.87-14.46,51.68s5.46,37.89,14.47,51.67c10.62,16.27,26.19,26.54,43.53,26.54c1.87,0,3.71-0.12,5.53-0.35 c10.711-1.37,21.468-1.922,32.219-1.708C122.465,400.629,111.945,391.903,104.117,379.91z"
                    })
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#E54728"
                    },
                    d: "M206.98,379.91c-3.51,5.36-7.54,10.07-12,13.97c-4.46-3.9-8.5-8.61-12-13.97 c-9.01-13.78-14.47-31.86-14.47-51.67s5.46-37.9,14.46-51.68c3.5-5.36,7.54-10.07,12-13.97c4.46,3.9,8.5,8.61,12,13.97 c-9,13.78-14.46,31.87-14.46,51.68S197.97,366.13,206.98,379.91z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#FFFFFF"
                    },
                    d: "M350.573,352.245c-0.519,0-1.045-0.055-1.573-0.167c-4.051-0.865-6.633-4.85-5.769-8.9 c1.042-4.878,1.569-9.903,1.569-14.938c0-22.068-12.013-47.721-27.5-47.721c-4.142,0-7.5-3.357-7.5-7.5s3.358-7.5,7.5-7.5 c23.038,0,42.5,28.723,42.5,62.721c0,6.085-0.639,12.164-1.899,18.068C357.148,349.832,354.035,352.245,350.573,352.245z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#90BC42"
                    },
                    d: "M330.81,7.5L221.82,116.48C221.82,56.29,270.62,7.5,330.81,7.5z"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("circle", {
                    style: {
                        fill: "#FFFFFF"
                    },
                    cx: "162.146",
                    cy: "271.047",
                    r: "7.5"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("circle", {
                    style: {
                        fill: "#FFFFFF"
                    },
                    cx: "341.928",
                    cy: "367.234",
                    r: "7.5"
                }),
                /*#__PURE__*/ jsx_runtime_.jsx("path", {
                    style: {
                        fill: "#333333"
                    },
                    d: "M364.339,268.464c-12.405-16.729-29.11-25.941-47.038-25.941c-9.591,0-18.874,2.662-27.385,7.781 c-2.522-17.369-9.186-49.232-26.461-83.616c-7.925-15.774-17.233-30.31-27.814-43.528C293.38,116.304,338.306,67.054,338.306,7.5 c0-4.143-3.358-7.5-7.5-7.5c-58.965,0-107.832,44.042-115.446,100.958c-3.446-3.306-6.98-6.513-10.625-9.589 c-30.045-25.358-66.834-42.82-109.347-51.901c-4.049-0.864-8.036,1.719-8.901,5.769c-0.865,4.051,1.717,8.036,5.768,8.901 c39.947,8.532,74.446,24.84,102.536,48.469c4.317,3.631,8.485,7.442,12.505,11.419c-27.697,19.155-50.757,49.787-67.033,89.22 c-8.008,19.401-12.693,36.721-15.196,47.702c-8.791-5.532-18.434-8.42-28.413-8.42c-17.929,0-34.635,9.212-47.041,25.938 c-11.907,16.055-18.464,37.281-18.464,59.77c0,22.493,6.558,43.723,18.464,59.779c12.405,16.728,29.111,25.939,47.041,25.939 c2.182,0,4.366-0.139,6.481-0.412c17.86-2.282,35.972-2.283,53.821-0.001c2.125,0.274,4.309,0.413,6.491,0.413 c16.205,0,31.646-7.749,43.538-21.572c11.963,13.941,27.233,21.571,43.524,21.571c2.16,0,4.34-0.139,6.483-0.412 c17.857-2.285,35.966-2.285,53.827,0c2.141,0.273,4.321,0.412,6.48,0.412c17.928,0,34.633-9.213,47.038-25.941 c11.907-16.056,18.464-37.284,18.464-59.773S376.246,284.52,364.339,268.464z M323.01,15.297 c-3.798,49.755-43.634,89.591-93.39,93.389C233.418,58.931,273.254,19.095,323.01,15.297z M217.707,125.093 c12.422,14.173,23.167,30.165,32.088,47.821c15.137,29.958,21.754,58.373,24.587,75.439c-7.529-3.841-15.585-5.831-23.872-5.831 c-16.292,0-31.563,7.631-43.526,21.574c-11.892-13.821-27.332-21.57-43.536-21.57c-7.639,0-15.081,1.695-22.095,4.967 c2.598-10.153,6.677-23.749,12.776-38.526C165.109,182.369,184.918,146.872,217.707,125.093z M163.448,398.953 c-1.542,0-3.08-0.098-4.58-0.291c-19.122-2.443-38.512-2.442-57.643,0.001c-1.491,0.192-3.028,0.29-4.571,0.29 c-27.849,0-50.505-31.725-50.505-70.719c0-38.988,22.657-70.708,50.505-70.708c10.067,0,19.828,4.225,28.227,12.218 c2.896,2.756,7.444,2.756,10.341,0c8.399-7.993,18.16-12.218,28.226-12.218c12.807,0,25.194,7.006,34.56,19.35 c-8.423,14.734-13,32.603-13,51.361c0,18.759,4.577,36.629,13.001,51.364C188.644,391.946,176.256,398.953,163.448,398.953z M317.301,398.952c-1.526,0-3.068-0.098-4.579-0.291c-9.562-1.224-19.188-1.835-28.817-1.835c-9.626,0-19.255,0.611-28.814,1.835 c-1.513,0.193-3.054,0.291-4.581,0.291c-14.519,0-27.617-8.635-36.838-22.417c-0.255-0.506-0.564-0.989-0.935-1.433 c-7.911-12.487-12.729-28.9-12.729-46.866c0-38.992,22.655-70.715,50.502-70.715c10.066,0,19.826,4.225,28.225,12.218 c2.896,2.756,7.444,2.755,10.341-0.001c8.398-7.992,18.158-12.217,28.225-12.217c27.847,0,50.502,31.723,50.502,70.715 S345.148,398.952,317.301,398.952z"
                })
            ]
        })
    })
});

;// CONCATENATED MODULE: ./model/Minigames/MinigameList/Panier.tsx






const NBROW = 7;
class PanierCaseTemplate {
    constructor(){
        this.hasFruit = false;
    }
}
const minigameInfos = {
    Controls (logicGrid) {
        return /*#__PURE__*/ (0,jsx_runtime_.jsxs)(jsx_runtime_.Fragment, {
            children: [
                /*#__PURE__*/ jsx_runtime_.jsx(react_.IconButton, {
                    "aria-label": "left",
                    icon: /*#__PURE__*/ jsx_runtime_.jsx(icons_.ArrowLeftIcon, {}),
                    onClick: ()=>this.playerInput("left", logicGrid)
                }),
                /*#__PURE__*/ jsx_runtime_.jsx(react_.IconButton, {
                    "aria-label": "right",
                    icon: /*#__PURE__*/ jsx_runtime_.jsx(icons_.ArrowRightIcon, {}),
                    onClick: ()=>this.playerInput("right", logicGrid)
                }),
                " "
            ]
        });
    },
    refreshInterval: 150,
    setUpdateGrid: ()=>null,
    setLogicGrid: ()=>null,
    nbRow: NBROW,
    nbCol: 5,
    score: 0,
    scoreTresh: 10,
    player: {
        position: {
            x: NBROW - 1,
            y: "A"
        },
        direction: [
            IOutput/* Direction.left */.Nm.left,
            IOutput/* Direction.right */.Nm.right
        ]
    },
    ViewGrid (logicGrid) {
        const cols = Array.from(logicGrid.keys());
        const parameters = [];
        for(let i = 0; i < this.nbRow; i++){
            for(let j = 0; j < this.nbCol; j++){
                parameters.push(i === this.nbRow - 1 && this.player.position.y === cols[j] ? /*#__PURE__*/ jsx_runtime_.jsx(Panier, {
                    boxSize: 12
                }) : logicGrid.get(cols[j])[i].hasFruit ? /*#__PURE__*/ jsx_runtime_.jsx(Cerise, {
                    boxSize: 12
                }) : /*#__PURE__*/ jsx_runtime_.jsx(jsx_runtime_.Fragment, {}));
            }
        }
        return /*#__PURE__*/ jsx_runtime_.jsx(react_.Grid, {
            templateColumns: `repeat(${this.nbCol},auto)`,
            sx: {
                height: "85vh",
                width: "min(80vh,80vh)"
            },
            children: parameters.map((v)=>/*#__PURE__*/ jsx_runtime_.jsx(react_.Center, {
                    children: /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                        colSpan: 1,
                        rowSpan: 1,
                        backgroundColor: "gray.500",
                        w: "42px",
                        h: "42px",
                        children: v
                    })
                }))
        });
    },
    init (logicGrid, setUpdateGrid, setLogicGrid) {
        for(let i = 0; i < 3; i++){
            spawnItems(logicGrid, this);
        }
        this.setUpdateGrid = setUpdateGrid;
        this.setLogicGrid = setLogicGrid;
    },
    update (logicGrid) {
        this.setLogicGrid(logicGrid);
        this.setUpdateGrid(true);
    },
    caseTemplateCreate: ()=>new PanierCaseTemplate(),
    playerInput (input) {
        let oldPos = this.player.position.y.charCodeAt(0) - "A".charCodeAt(0);
        if (input === "left" && oldPos > 0) {
            oldPos--;
        } else if (input === "right" && oldPos < 4) {
            oldPos++;
        }
        this.player.position.y = String.fromCharCode("A".charCodeAt(0) + oldPos);
        this.setUpdateGrid(true);
    },
    evolve (logicGrid) {
        const logicGridCopy = new Map(JSON.parse(JSON.stringify(Array.from(logicGrid))));
        logicGrid.forEach((row, i)=>{
            row.forEach((cell, j)=>{
                let realCell = cell;
                if (realCell.hasFruit) {
                    if (j === this.nbRow - 1) {
                        this.score += this.player.position.y === i ? 1 : 0;
                    } else {
                        logicGridCopy.get(i)[j + 1].hasFruit = true;
                    }
                    logicGridCopy.get(i)[j].hasFruit = false;
                    realCell.hasFruit = false;
                }
            });
        });
        Math.random() < 1 / 3 ? spawnItems(logicGridCopy, this) : null;
        this.update(logicGridCopy);
    }
};
function spawnItems(logicGrid, minigame) {
    const letterDrawn = String.fromCharCode(65 + Math.floor(Math.random() * minigame.nbCol));
    const caseDrawn = logicGrid.get(letterDrawn)[0];
    caseDrawn.hasFruit = true;
}


/***/ }),

/***/ 8870:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
// ESM COMPAT FLAG
__webpack_require__.r(__webpack_exports__);

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "RPSCaseTemplate": () => (/* binding */ RPSCaseTemplate),
  "minigameInfos": () => (/* binding */ minigameInfos)
});

// EXTERNAL MODULE: external "react/jsx-runtime"
var jsx_runtime_ = __webpack_require__(997);
// EXTERNAL MODULE: external "@chakra-ui/react"
var react_ = __webpack_require__(8930);
// EXTERNAL MODULE: external "@chakra-ui/icons"
var icons_ = __webpack_require__(4513);
;// CONCATENATED MODULE: ./components/icons/RPSIcons.tsx


const RockIcon = (0,icons_.createIcon)({
    displayName: "RockIcon",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#000",
            d: "M132.4 40.51c-28.4-.1-56.21 46.13-46.13 72.89 11.98 31.7 76.43 53.3 97.93 27.1 23.6-28.9-13.2-97.88-50.4-99.95h-1.4zm-.5 18.84h.9c2.6.1 5.9.55 6.5 3.23 2.7 12.38-26.4 49.82-31.3 47.82-9.58-3.9-6.9-23.04-1.6-32.17 4.6-8.19 16.6-18.57 25.5-18.88zm234.9 1.01c-48 .59-111.6 26.22-111 76.84.7 68.2 99.7 95.4 155.9 84.1 28-5.5 62.5-35.7 61.9-69.6-.8-46-49.3-84.06-87.4-90.06-6-.94-12.5-1.36-19.4-1.28zm-39.7 25.05c5.5-.1 15.1 1.72 12.7 6.45-9.6 19.04-47.6 63.64-56.4 54.14-9.8-10.5-3-32 6.7-42.5 8-8.64 24.7-17.96 37-18.09zm-162 111.49c-9.5.2-18.3 2.5-25.6 7.3-21.5 14.2-27.7 71.8-12.2 96.6C157 348 253 357.6 263.8 297.1c9.3-52-52.9-100.9-98.7-100.2zm2.6 12.1c3.2-.1 7.5 1.3 7.7 5.6 1 19.8-21.9 67.4-34.3 57.7-8.3-6.5-4.1-31.4 1.8-42.6 5.2-9.6 16.5-20.6 24.8-20.7zm196.8 55.8c-6.4 0-12.3 1-17.6 3.2-29.4 12.1-33.4 67.4-9.3 86.6 27.9 22.6 102.6 11.7 110.1-21.4 7.1-31.4-44.7-68.3-83.2-68.4zm-1 9.1c2 .1 4.8.4 5.4 2.8 3.6 14.4-10.7 60.3-24.1 56.7-9.6-2.6-9.1-24.4-5.2-35.2 3.9-10.3 15.3-24.6 23.9-24.3zm-263.4 65.5c-36.1-.2-73.09 58.2-58.43 91.2C59.32 470.3 146.1 488 170.2 451.9c24.5-36.8-26-112.2-70.1-112.5zm4.2 14.3c4.5-.1 10.2 1.9 9.8 7.6-1.8 26-35.65 77.1-48.52 63.1-8.54-9.3-.89-37.3 7.65-49.7 7.11-10 21.23-20.9 31.07-21z"
        })
    })
});
const PaperIcon = (0,icons_.createIcon)({
    displayName: "PaperIcon",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#000",
            d: "M480 40L32 296l112.148 37.383L448 72 209.404 355.135 320 392 480 40zM208 376l-16 96 49.932-83.863L208 376z"
        })
    })
});
const ScissorsIcon = (0,icons_.createIcon)({
    displayName: "ScissorsIcon",
    viewBox: "0 0 800 800",
    // path can also be an array of elements, if you have multiple paths, lines, shapes, etc.
    path: /*#__PURE__*/ jsx_runtime_.jsx("svg", {
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: "0 0 512 512",
        children: /*#__PURE__*/ jsx_runtime_.jsx("path", {
            fill: "#000",
            d: "M478.89 21.842c-6.434-.113-15.047 3.628-24.242 12.822L328.912 160.4c0-.007.003-.015.004-.023l-60.228 60.228.01.01-12.727 12.727-.01-.01-22.657 22.658.01.01-12.44 12.44-.01-.01-30.884 30.884c0 .005.002.01.004.014l-55.656 55.656 22.627 22.627 320.32-320.32c18.827-18.825 14.79-35.217 1.616-35.448zm-445.84.06c-13.176.232-17.213 16.623 1.614 35.45l185.912 185.912 22.658-22.66-44.164-44.16c0 .02 0 .038-.002.058L57.29 34.725C48.098 25.53 39.484 21.79 33.05 21.902zm234.538 36.39c-9.308 42.14-12.624 79.314-27.922 121.454-21.636-16.89-30.623-41.47-43.836-63.414.586 10.88 1.66 22.256 2.42 33.838l57.71 57.71 78.485-78.485a2474.362 2474.362 0 0 1 4.078-21.237c-21.27 28.796-42.452 44.68-71.396 73.475-6.752-45.418.278-77.923.46-123.34zM87.873 170.623c-.052.003-.08.017-.082.044-.258 5.25 104.427 78.357 96.222 83.578-13.465 8.55-57.975 31.11-83.508 44.914 19.72-3.345 53.538-7.933 72.18-8.002l35.453-35.453-63.655-63.652c-27.027-10.216-54.915-21.568-56.61-21.43zm245.143 56.144l-29.086 29.086 56.174 56.17c10.848.735 21.692 1.416 32.5 1.218-24.518-15.9-71.99-43.734-71.64-50.568.353-6.887 63.447-21.216 95.518-31.606-27.787-2.137-55.973 3.846-83.466-4.3zm-41.813 41.812l-22.656 22.656 86.437 86.438 22.627-22.627-44.473-44.475h.06l-41.994-41.992zm-35.098 35.098l-57.703 57.703c.408 14.207.21 28.454-.52 40.38 11.322-26.84 17.846-51.86 45.827-74.473 14.444 11.985 24.816 40.267 37.224 60.4-.91-19.042-6.873-52.002-5.03-64.214l-19.8-19.797zM111.7 354.984L98.974 367.71l45.256 45.257 12.725-12.73-45.252-45.253h-.002zm288.538.06l-45.254 45.253v.002l12.727 12.727 45.257-45.256-12.73-12.725zM94.023 385.39l-9.9 9.9 32.527 32.526 9.9-9.9-32.527-32.527zm323.893.06l-32.527 32.527 9.9 9.9 32.526-32.527-9.9-9.9zm-346.52 22.568L21.9 457.512l32.528 32.527 10.603-10.604 10.73 10.726 17.847-17.848-10.728-10.726 21.042-21.043-9.9-9.9-38.89 38.89-12.728-12.728 38.89-38.89-9.9-9.9zm369.15.06l-9.9 9.9 38.89 38.89-12.73 12.728-38.89-38.89-9.898 9.9 49.494 49.494 32.527-32.528-10.604-10.603 10.726-10.73-17.848-17.847-10.726 10.728-21.043-21.042z"
        })
    })
});

;// CONCATENATED MODULE: ./model/Minigames/MinigameList/RPS.tsx



var RPSEnum;
(function(RPSEnum) {
    RPSEnum[RPSEnum["Rock"] = 0] = "Rock";
    RPSEnum[RPSEnum["Paper"] = 1] = "Paper";
    RPSEnum[RPSEnum["Scissors"] = 2] = "Scissors";
    RPSEnum[RPSEnum["None"] = -1] = "None";
})(RPSEnum || (RPSEnum = {}));
const rpsIcon = [
    /*#__PURE__*/ jsx_runtime_.jsx(RockIcon, {}),
    /*#__PURE__*/ jsx_runtime_.jsx(PaperIcon, {}),
    /*#__PURE__*/ jsx_runtime_.jsx(ScissorsIcon, {})
];
class RPSCaseTemplate {
    whatElement = -1;
    constructor(){}
}
let viewClick = false;
const minigameInfos = {
    Controls (logicGrid) {
        return /*#__PURE__*/ jsx_runtime_.jsx(jsx_runtime_.Fragment, {
            children: Object.keys(RPSEnum).filter((k)=>typeof RPSEnum[k] === "number" && k != "None").map((elem)=>/*#__PURE__*/ jsx_runtime_.jsx(react_.IconButton, {
                    "aria-label": elem,
                    icon: rpsIcon[RPSEnum[elem]],
                    onClick: ()=>this.playerInput(elem, logicGrid)
                }))
        });
    },
    refreshInterval: 99999999999999,
    setUpdateGrid: ()=>null,
    setLogicGrid: ()=>null,
    nbRow: 1,
    nbCol: 2,
    score: 0,
    scoreTresh: 5,
    player: {
        position: {
            x: 0,
            y: "A"
        },
        direction: []
    },
    ViewGrid (logicGrid) {
        return /*#__PURE__*/ (0,jsx_runtime_.jsxs)(react_.Grid, {
            templateColumns: `repeat(${this.nbCol},auto)`,
            sx: {
                height: "80vh",
                width: "min(80vh,80vh)"
            },
            children: [
                /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                    colSpan: 1,
                    rowSpan: 1,
                    w: "42px",
                    h: "42px",
                    children: logicGrid.get("A")[0].whatElement !== -1 ? rpsIcon[logicGrid.get("A")[0].whatElement] : /*#__PURE__*/ jsx_runtime_.jsx(jsx_runtime_.Fragment, {})
                }),
                /*#__PURE__*/ jsx_runtime_.jsx(react_.GridItem, {
                    colSpan: 1,
                    rowSpan: 1,
                    w: "42px",
                    h: "42px",
                    children: logicGrid.get("B")[0].whatElement !== -1 ? rpsIcon[logicGrid.get("B")[0].whatElement] : /*#__PURE__*/ jsx_runtime_.jsx(jsx_runtime_.Fragment, {})
                })
            ]
        });
    },
    init (logicGrid, setUpdateGrid, setLogicGrid) {
        this.setUpdateGrid = setUpdateGrid;
        this.setLogicGrid = setLogicGrid;
    },
    update (logicGrid) {
        this.setLogicGrid(logicGrid);
        this.setUpdateGrid(true);
    },
    caseTemplateCreate: ()=>new RPSCaseTemplate(),
    playerInput (input, logicGrid) {
        const player = logicGrid.get("A")[0];
        const ia = logicGrid.get("B")[0];
        if (viewClick) {
            // Reset
            player.whatElement = RPSEnum.None;
            ia.whatElement = RPSEnum.None;
        } else {
            // Play and play result
            player.whatElement = Object.keys(RPSEnum).findIndex((e)=>e == input);
            ia.whatElement = Math.floor(Math.random() * 3);
            if ((player.whatElement - ia.whatElement) % 3 == 1) {
                this.score += 1;
            }
        }
        viewClick = !viewClick;
        this.update(logicGrid);
        this.setUpdateGrid(true);
    },
    evolve () {}
};


/***/ }),

/***/ 7179:
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "minigameInfos": () => (/* binding */ minigameInfos)
/* harmony export */ });
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(6689);
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);

const minigameInfos = {
    Controls () {
        return undefined;
    },
    refreshInterval: 0,
    setLogicGrid () {},
    evolve () {},
    setUpdateGrid: ()=>null,
    ViewGrid () {
        return null;
    },
    caseTemplateCreate () {
        return null;
    },
    player: null,
    playerInput () {},
    nbCol: 0,
    nbRow: 0,
    score: 0,
    scoreTresh: 0,
    init () {},
    update () {}
};


/***/ }),

/***/ 8703:
/***/ ((module, __webpack_exports__, __webpack_require__) => {

"use strict";
__webpack_require__.a(module, async (__webpack_handle_async_dependencies__, __webpack_async_result__) => { try {
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "default": () => (/* binding */ minigame),
/* harmony export */   "getStaticProps": () => (/* binding */ getStaticProps)
/* harmony export */ });
/* harmony import */ var react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(997);
/* harmony import */ var react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _model_Minigames_MinigameController__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(791);
/* harmony import */ var _chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(8930);
/* harmony import */ var _chakra_ui_react__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _components_minigame_MinigameComponent__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(477);
var __webpack_async_dependencies__ = __webpack_handle_async_dependencies__([_components_minigame_MinigameComponent__WEBPACK_IMPORTED_MODULE_3__]);
_components_minigame_MinigameComponent__WEBPACK_IMPORTED_MODULE_3__ = (__webpack_async_dependencies__.then ? (await __webpack_async_dependencies__)() : __webpack_async_dependencies__)[0];




async function getStaticProps() {
    return {
        props: {
            prepareMinigameLists: await (0,_model_Minigames_MinigameController__WEBPACK_IMPORTED_MODULE_1__/* .getStaticPropsMinigame */ .Z)()
        }
    };
}
function minigame({ prepareMinigameLists  }, state) {
    _model_Minigames_MinigameController__WEBPACK_IMPORTED_MODULE_1__/* .MinigameController.init */ .l.init(prepareMinigameLists);
    return /*#__PURE__*/ (0,react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsxs)(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__.Grid, {
        templateColumns: "repeat(8,1fr)",
        gap: 6,
        children: [
            /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__.GridItem, {
                colSpan: 1
            }),
            /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__.GridItem, {
                rowSpan: 2,
                colSpan: 6
            }),
            /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__.GridItem, {
                colSpan: 1
            }),
            /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__.GridItem, {
                rowSpan: 6,
                colSpan: 1
            }),
            /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_chakra_ui_react__WEBPACK_IMPORTED_MODULE_2__.GridItem, {
                rowSpan: 6,
                colSpan: 6,
                children: /*#__PURE__*/ react_jsx_runtime__WEBPACK_IMPORTED_MODULE_0__.jsx(_components_minigame_MinigameComponent__WEBPACK_IMPORTED_MODULE_3__/* ["default"] */ .Z, {})
            })
        ]
    });
}

__webpack_async_result__();
} catch(e) { __webpack_async_result__(e); } });

/***/ }),

/***/ 4513:
/***/ ((module) => {

"use strict";
module.exports = require("@chakra-ui/icons");

/***/ }),

/***/ 8930:
/***/ ((module) => {

"use strict";
module.exports = require("@chakra-ui/react");

/***/ }),

/***/ 1853:
/***/ ((module) => {

"use strict";
module.exports = require("next/router");

/***/ }),

/***/ 6689:
/***/ ((module) => {

"use strict";
module.exports = require("react");

/***/ }),

/***/ 997:
/***/ ((module) => {

"use strict";
module.exports = require("react/jsx-runtime");

/***/ }),

/***/ 628:
/***/ ((module) => {

"use strict";
module.exports = import("@firebase/database");;

/***/ }),

/***/ 7147:
/***/ ((module) => {

"use strict";
module.exports = require("fs");

/***/ })

};
;

// load runtime
var __webpack_require__ = require("../webpack-runtime.js");
__webpack_require__.C(exports);
var __webpack_exec__ = (moduleId) => (__webpack_require__(__webpack_require__.s = moduleId))
var __webpack_exports__ = __webpack_require__.X(0, [538], () => (__webpack_exec__(8703)));
module.exports = __webpack_exports__;

})();