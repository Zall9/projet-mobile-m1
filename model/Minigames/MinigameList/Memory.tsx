...
export const minigameInfos: IMinigame = {
    ...
    ViewGrid(logicGrid: GridMinigame) {
        ...
        for (let i = 0; i < this.nbRow; i++) {
            for (let j = 0; j < this.nbCol; j++) {
                const observedCell = (logicGrid.get(cols[j]) as MemoryCaseTemplate[])[i];
                parameters.push(
        ...
        return (
            ...
            >
                {parameters.map((v, index) => (
                    ...
                    >
                        {<Image
                            onClick={() => this.playerInput(transformCoord(index, this.nbCol))}/>}
                    </GridItem>
                ))}
            </Grid>
        );
    ...
    init(
        ...
        const values = Array.from({length: this.nbRow * this.nbCol / 2}, (_, i) => i + 1);
        values.push(...Array.from({length: this.nbRow * this.nbCol / 2}, (_, i) => i + 1));
        ...
        logicGrid.forEach((row, i) => {
            row.forEach((cell, j) => {
                ...
                realCell.idLink = values[i.charCodeAt(0) - "A".charCodeAt(0) + this.nbCol * j]
            });
        });
    ...
    playerInput(input: string, logicGrid: GridMinigame): void {
        ...
        const returnedCell = (logicGrid.get(input[0]) as MemoryCaseTemplate[])[parseInt(input[2])];
